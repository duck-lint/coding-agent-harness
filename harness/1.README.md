...
- Keep planning and implementation bounded to the user's current goal.
...
- Keep the harness light for small work and structured for multi-step or risky work.
...
- Track only the current user-authorized implementation goal. Do not pre-plan future bundles unless the user supplies the next end goal.
...
- `2.sub-agent-assignment-template.md`: handoff packet for assigning bounded work to another agent.
...
For small one-off work, skip project scaffolding and use a lightweight chat plan.
...
Optional evidence, decision, seam, or verification files can use the same `implementation-XX` prefix when the work needs them. Keep the bundle small; create extra files only when they reduce real resumption or verification risk.
...
Escalate before changing schema, API, auth, storage, deployment, billing, destructive operations, broad architecture, compatibility promises, or behavior whose safest path depends on project intent rather than local mechanics.

Approval must name the boundary and scope. Approval for one schema change is not approval for deployment. Approval for a local refactor is not approval for a compatibility layer.
