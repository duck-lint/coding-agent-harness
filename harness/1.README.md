...
Escalate before changing schema, API, auth, storage, deployment, billing, destructive operations, broad architecture, compatibility promises, or project-intent-dependent behavior not covered by the repo spec or current user authorization.

Approval must name the boundary and current project-spec alignment frame. Approval for one schema change is not approval for deployment. Approval for a local refactor is not approval for a compatibility layer.
